...
class Game {
    constructor() {
        ...
        // 3 ennemis dispersés sur la carte
        this.enemies = [
            {
                x: 4,
                y: 2,
        ...
        // Champignon qui restaure la vie
        this.mushroom = {
            x: 3,
            y: 5,
            symbol: '🍄',
            ...
            if (enemy.alive) {
                ...
                if ((dx === 1 && dy === 0) || (dx === 0 && dy === 1)) {
                    ...
                    if (this.hero.health <= 0) {
                        ...
                        alert('💀 Game Over! Les aliens t\'ont vaincu!');
                        setTimeout(() => {
    ...
    checkMushroomConsumption() {
        if (!this.mushroom.consumed && 
            this.hero.x === this.mushroom.x && 
            this.hero.y === this.mushroom.y) {
            
            // Restaurer 1 point de vie (max 3)
            if (this.hero.health < this.hero.maxHealth) {
                this.hero.health++;
                this.updateHealthBar();
                this.mushroom.consumed = true;
                this.score += 50;
                document.getElementById('score').textContent = this.score;
...
function addCommand(direction) {
    ...
    if (newX >= 0 && newX < game.cols && newY >= 0 && newY < game.rows && !game.isWall(newX, newY)) {
        ...
        if (enemyAtPosition) {
            alert('⚠️ Un alien bloque le passage! Tu dois le vaincre d\'abord!');
            return;
        }
        ...
        if (game.hero.x === game.gem.x && game.hero.y === game.gem.y) {
            if (!game.areAllEnemiesDead()) {
                ...
                localStorage.setItem('codecombat_level', '7');
                localStorage.setItem('codecombat_score', game.score);
                
                // Marquer le niveau 6 comme complété
                ...
                setTimeout(() => {
                    window.location.href = 'victoire.html?score=' + game.score + '&level=6';
                }, 500);
            }
        }
    }
...
function performAttack() {
    ...
    if (!attackedEnemy) {
        alert('⚠️ Il n\'y a pas d\'alien à portée! Rapproche-toi d\'un alien pour l\'attaquer!');
        return;
    }
    ...
    if (attackedEnemy.health <= 0) {
        ...
        setTimeout(() => {
            ...
            if (game.areAllEnemiesDead()) {
                ...
                game.score += 200;
                document.getElementById('score').textContent = game.score;
                alert('🎉 Tous les aliens sont vaincus! +200 points bonus! Va chercher la gemme!');
            }
        }, 500);
    } else {
        game.draw();
